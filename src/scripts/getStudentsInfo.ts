...
const getCourseByDepartment = async (htmlData: any) => {
  ...
  $("tr", tableBody).map((index, elem) => {
    ...
    const courseId = getCourseId(courseCode);
...
(async () => {
  ...
  const result = await Promise.all(
    map(TERMS, async (term) => {
      ...
      console.log(`*** get list departments of ${term.termName} ${moment().from(startAt)} ***`);
      ...
      const deptData = await Promise.all(
        map(dataDept, async (dept) => {
          ...
          console.log(`*** get list course of ${dept.deptName} ${moment().from(startAt)} ***`);
          ...
          const courseData = await Promise.all(
            map(dataCourse, async (c) => {
              ...
              console.log(`*** get list classes of ${c.courseName} ${moment().from(startAt)} ***`);
              ...
              const studentOfAClass = await Promise.all(
                map(dataClass, async (cl: any) => {
                  ...
                  console.log(`*** get list student of ${cl.className} ${moment().from(startAt)} ***`);
      ...
      console.log("***Done at***: ", moment().from(startAt));
    })
  );
})();
